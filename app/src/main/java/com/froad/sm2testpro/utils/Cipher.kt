...
class Cipher {
    ...
    private fun Reset() {
        ...
        sm3c3 = SM3Digest()
        var p = FCharUtils.byteConvert32Bytes(p2!!.x.toBigInteger())
        sm3keybase!!.update(p, 0, p!!.size)
        sm3c3!!.update(p, 0, p!!.size)
        p = FCharUtils.byteConvert32Bytes(p2!!.y.toBigInteger())
        sm3keybase!!.update(p, 0, p!!.size)
        ct = 1
        NextKey()
    }
    ...
    fun Dofinal(c3: ByteArray?) {
        val p = FCharUtils.byteConvert32Bytes(p2!!.y.toBigInteger())
        sm3c3!!.update(p, 0, p!!.size)
        sm3c3!!.doFinal(c3, 0)
        Reset()
    }
